...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (empty($name)) {
        ...
        try {
            ...
            $stmt_student_update = $conn->prepare($update_student_query);
            $stmt_student_update->bind_param(
                "sssssssssssi",
                $name,
                $phone,
                $date_of_birth,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_notifications'])) {
    ...
    $class_announcements = isset($_POST['class_announcements']) ? 1 : 0;

    // Check if student_preferences table exists
    $table_exists_query = "SHOW TABLES LIKE 'student_preferences'";
    $table_result = $conn->query($table_exists_query);

    if ($table_result->num_rows > 0) {
        // Get existing preferences or insert new
        $check_prefs_query = "SELECT id FROM student_preferences WHERE student_id = ?";
        $stmt_check_prefs = $conn->prepare($check_prefs_query);
        $stmt_check_prefs->bind_param("i", $student_id);
...
$student_preferences = [
    ...
    'grade_updates' => 1,
    'class_announcements' => 1
];

// Check if student_preferences table exists before querying
$table_exists_query = "SHOW TABLES LIKE 'student_preferences'";
$table_result = $conn->query($table_exists_query);

if ($table_result->num_rows > 0) {
    $prefs_query = "SELECT * FROM student_preferences WHERE student_id = ?";
    $stmt_prefs = $conn->prepare($prefs_query);
    $stmt_prefs->bind_param("i", $student_id);
    $stmt_prefs->execute();
    $prefs_result = $stmt_prefs->get_result();
    if ($prefs_result->num_rows > 0) {
        $student_preferences = $prefs_result->fetch_assoc();
    }
}

// Get student privacy settings if they exist
$privacy_settings = [
    'profile_visibility' => 'students_only',
    'show_progress_to_teachers' => 1,
    'show_grades_to_teachers' => 1
];

// Check if student_privacy table exists
$table_exists_query = "SHOW TABLES LIKE 'student_privacy'";
$table_result = $conn->query($table_exists_query);

if ($table_result->num_rows > 0) {
    $privacy_query = "SELECT * FROM student_privacy WHERE student_id = ?";
    $stmt_privacy = $conn->prepare($privacy_query);
    $stmt_privacy->bind_param("i", $student_id);
...
// Get student system preferences if they exist
$system_preferences = [
    'theme' => 'light',
    'language' => 'en',
    'timezone' => 'UTC',
...
// Check if student_system_preferences table exists
$table_exists_query = "SHOW TABLES LIKE 'student_system_preferences'";
$table_result = $conn->query($table_exists_query);

if ($table_result->num_rows > 0) {
    $system_query = "SELECT * FROM student_system_preferences WHERE student_id = ?";
    $stmt_system = $conn->prepare($system_query);
    $stmt_system->bind_param("i", $student_id);
    $stmt_system->execute();
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="grade_level" value="<?php echo htmlspecialchars($student['grade_level'] ?? ''); ?>"
                                                class="w-full form-input px-4 py-3 rounded-lg"
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <input type="text" name="school_name" value="<?php echo htmlspecialchars($student['school_name'] ?? ''); ?>"
                                                class="w-full form-input px-4 py-3 rounded-lg"
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <input type="tel" name="parent_phone" value="<?php echo htmlspecialchars($student['parent_phone'] ?? ''); ?>"
                                                class="w-full form-input px-4 py-3 rounded-lg">
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <textarea name="bio" rows="4"
                                            class="w-full form-input px-4 py-3 rounded-lg"
                                            placeholder="Tell us about your interests and goals..."><?php echo htmlspecialchars($student['bio'] ?? ''); ?></textarea>
                                    </div>
                ...
                <div class="card p-6">
                    ...
                    <form method="POST" action="">
                        <div class="space-y-6">
                            ...
                            <div class="flex items-center justify-between py-4 border-b">
                                ...
                                <label class="switch">
                                    <input type="checkbox" name="email_notifications"
                                        <?php echo $student_preferences['email_notifications'] ? 'checked' : ''; ?>>
                            ...
                            <div class="flex items-center justify-between py-4 border-b">
                                ...
                                <label class="switch">
                                    <input type="checkbox" name="assignment_alerts"
                                        <?php echo $student_preferences['assignment_alerts'] ? 'checked' : ''; ?>>
                                    <span class="slider"></span>
                                </label>
                            </div>
                            ...
                            <div class="flex items-center justify-between py-4 border-b">
                                <div>
                                    <h4 class="font-semibold text-gray-800">Deadline Reminders</h4>
                                    <p class="text-sm text-gray-600">Reminders for upcoming assignment deadlines</p>
                                </div>
                                <label class="switch">
                                    <input type="checkbox" name="deadline_reminders"
                                        <?php echo $student_preferences['deadline_reminders'] ? 'checked' : ''; ?>>
                                    <span class="slider"></span>
                                </label>
                            </div>
                            ...
                            <div class="flex items-center justify-between py-4 border-b">
                                <div>
                                    <h4 class="font-semibold text-gray-800">Grade Updates</h4>
                                    <p class="text-sm text-gray-600">Notifications when grades are posted</p>
                                </div>
                                <label class="switch">
                                    <input type="checkbox" name="grade_updates"
                                        <?php echo $student_preferences['grade_updates'] ? 'checked' : ''; ?>>
                                    <span class="slider"></span>
                                </label>
                            </div>
                            ...
                            <div class="flex items-center justify-between py-4">
                                <div>
                                    <h4 class="font-semibold text-gray-800">Class Announcements</h4>
                                    <p class="text-sm text-gray-600">Announcements from your teachers</p>
                                </div>
                                <label class="switch">
                                    <input type="checkbox" name="class_announcements"
                                        <?php echo $student_preferences['class_announcements'] ? 'checked' : ''; ?>>
                                    <span class="slider"></span>
                                </label>
                            </div>
                    ...
                    <form method="POST" action="">
                        <div class="space-y-8">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <label for="profile_students" class="flex-1">
                                            ...
                                            <div class="text-sm text-gray-600">Only other students can see your profile</div>
                                        </label>
                                    </div>
                                    <div class="flex items-center">
                            ...
                            <div class="flex items-center justify-between py-4 border-t">
                                <div>
                                    <h4 class="font-semibold text-gray-800">Show Progress to Teachers</h4>
                                    <p class="text-sm text-gray-600">Allow teachers to see your learning progress</p>
                                </div>
                                <label class="switch">
                                    <input type="checkbox" name="show_progress_to_teachers"
                                        <?php echo $privacy_settings['show_progress_to_teachers'] ? 'checked' : ''; ?>>
                                    <span class="slider"></span>
                                </label>
                            </div>
                            ...
                            <div class="flex items-center justify-between py-4 border-t">
                                <div>
                                    <h4 class="font-semibold text-gray-800">Show Grades to Teachers</h4>
                                    <p class="text-sm text-gray-600">Allow teachers to see your grades</p>
                                </div>
                                <label class="switch">
                                    <input type="checkbox" name="show_grades_to_teachers"
                                        <?php echo $privacy_settings['show_grades_to_teachers'] ? 'checked' : ''; ?>>
                                    <span class="slider"></span>
                                </label>
                            </div>
    ...
    <script>
        ...
        function resetNotificationForm() {
            if (confirm('Reset all notification preferences to default values?')) {
                ...
                document.querySelectorAll('input[name="deadline_reminders"]')[0].checked = true;
                document.querySelectorAll('input[name="grade_updates"]')[0].checked = true;
                document.querySelectorAll('input[name="class_announcements"]')[0].checked = true;
            }
        }
        ...
        function resetPrivacyForm() {
            if (confirm('Reset all privacy settings to default values?')) {
                ...
                document.querySelectorAll('input[name="show_progress_to_teachers"]')[0].checked = true;
                document.querySelectorAll('input[name="show_grades_to_teachers"]')[0].checked = true;
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (hash && ['profile', 'security', 'notifications', 'privacy', 'system'].includes(hash)) {
                showTab(hash);
                // Update tab button
                document.querySelectorAll('.tab-button').forEach(button => {
